...
lemma dist_proj_lt_dist' {L : AffineSubspace ℝ Plane} {p z : Plane}
  ...
  -- Pythagorean: dist(p, z)² = dist(F, z)² + dist(p, F)²
  have h_pythag : dist p z ^ 2 = dist F z ^ 2 + dist p F ^ 2 := by
    have h := EuclideanGeometry.dist_sq_eq_dist_orthogonalProjection_sq_add_dist_orthogonalProjection_sq
      (s := L) z hz
    simp only [sq] at h ⊢
    linarith [h, dist_comm z p, dist_comm z F, dist_comm p z, dist_comm F z]
